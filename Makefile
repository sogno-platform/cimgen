...
run:
	...
	cd cimpp && find . -iname "*.hpp" | grep -v assignments | grep -v Folders | grep -v Task | grep -v IEC61970 | grep -v ConformLoad | grep -v Factory | grep -v BaseClass | sed "s/\.\///" | sed "s/\.hpp//" | sed "s/\(.*\)/    std::make_pair(\"\1\", \&&_factory),/" >> IEC61970.hpp
	cd cimpp && echo "};\n#endif" >> IEC61970.hpp
