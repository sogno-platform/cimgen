    ...
    for template_info in templates:
        ...
        if not os.path.exists(class_file):
            with open(class_file, 'w') as file:
                template_path = os.path.join(os.getcwd(), 'java/templates', template_info["filename"])
...
def create_assign(text, render):
    ...
    if _class != "String":
        ...
        """.replace("CLASS", _class).replace("LABEL", attribute_json["label"])
    else:
        assign = """
...
def resolve_headers(outputPath):
    ...
    _create_header_include_file(outputPath, "CIMClassMap.java", class_list_header, class_list_footer, "        ", "()", class_blacklist)
